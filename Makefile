...
$(FLITEDIR)/flite: $(FLITEDIR)/flite-2.0.0-release.tar.bz2
	cd $(FLITEDIR) && \
		...
		ln -s flite-2.0.0-release flite && \
		cd flite && \
		./configure --with-pic --with-audio=none --with-mmap=none && \
		make
...
$(FLITEDIR)/cmu_us_aup.flitevox:
	mkdir -p $(FLITEDIR)
	cd $(FLITEDIR) && \
		wget "http://www.festvox.org/flite/packed/flite-2.0/voices/cmu_us_aup.flitevox";

build: depflite
	goimports -w .
	go build -o bin/say cli/*.go
